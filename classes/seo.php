...
class seo {
     ...
     * @return seo
     */
    public static function get(moodle_url|string $url, ?renderer_base $output = null): seo {
        global $OUTPUT;
    ...
    public static function is_testing(): bool {
        ...
        return strstr(self::$useragent, '(Moodle SEO)') || strstr(self::$useragent, 'MoodleThemeSEO') || optional_param('ignore_seo_check', false, PARAM_BOOL);
    }
    ...
    public function pre_head_html(): string {
        ...
        try {
            ...
            if (self::is_testing() // During internal test for the publicity of the page.
                || !$this->is_crawler_allowed() // If the page is not public so no need to add these meta tags where no crawlers are allowed.
            ...
            $iscoursepage = in_array($pageurlpath, ['/course/view.php', '/enrol/index.php']) || $this->context->contextlevel == CONTEXT_COURSE;
            ...
            if ($iscoursepage) {
                ...
                $this->blog($output);
            } else if (class_exists('\local_page\context\page') && $this->context->contextlevel == \local_page\context\page::LEVEL) {
                $this->page($output);
            }
    ...
    private function course(&$output = '') {
        ...
        if ($course) {
            ...
            $output .= '<script type="application/ld+json">' . json_encode($courseschema, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . '</script>';
        }
    }
